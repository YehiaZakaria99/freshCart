...
// <VerificationInput validChars="0-9" inputProps={{ inputMode: "numeric" }} />
    ...
    try{
      setIsLoading(true)
      let {data:{message}} = await axios.post(`https://ecommerce.routemisr.com/api/v1/auth/forgotPasswords` , values)
  ...
  return (
    <>
{  !verifyEmail &&    <div className="box px-8 md:px-0 md:w-custom-width w-full mx-auto py-20 absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2">
        <h3 className="text-lg md:w-custom-width w-full mx-auto mb-4  ">Enter your email to reset your password :</h3>
        <div className="form ">
            ...
            <div className="md:w-custom-width w-full mx-auto flex gap-2 items-center">
              {
                ...
                :
                <>
                  <button type="submit" className="bg-main text-light py-1 px-2 rounded-md "  >Submit</button>
                  <Link to={"/"} className="bg-red-800 text-light py-1 px-2 rounded-md "  >Cancel</Link>
                </>
              }
            </div>
          </form>
